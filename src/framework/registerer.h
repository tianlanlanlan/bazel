...
#define REGISTER_ALIAS(TYPE, NAME, ALIAS) REGISTER_ALIAS_AT(__LINE__, TYPE, NAME, ALIAS)
#define REGISTER_ALIAS_AT(LINE, TYPE, NAME, ALIAS)                                                                     \
    Registry<TYPE>::Injector CONCAT_TOKENS(_xd_injector, LINE)(                                                        \
        ALIAS, []() { return Registry<TYPE>::New(NAME).release(); }, __FILE__, STRINGIFY(LINE));                       \
...
template <typename T, class... Args> class Registry {
  public:
    ...
    static bool CanNew(const std::string &key, Args... args) { return GetEntry(key, args...).first; }
    ...
    template <typename C> static const char *GetKeyFor() {
        return C::_xd_key(static_cast<const T *>(nullptr), std::function<void(Args...)>());
    }
    ...
    static std::vector<std::string> GetKeysWithLocations() {
        ...
        registry_mutex_.lock();
        for (const auto &iter : *GetRegistry()) {
            keys.emplace_back(std::string(iter.second.file) + ":" + std::string(iter.second.line) + ": " + iter.first);
        }
        for (const auto &iter : *GetInjectors()) {
            keys.emplace_back(std::string(iter.second.file) + ":" + std::string(iter.second.line) + ": " + iter.first +
                              "*");
    ...
    struct Injector {
        ...
        Injector(const std::string &key, const std::function<T *(Args...)> &function, const char *file = "undefined",
                 const char *line = "undefined")
            : key(key) {
            registry_mutex_.lock();
    ...
    struct Registerer {
        Registerer(function_t function, const std::string &key, const char *file, const char *line) {
            const Entry entry = {file, line, function};
            registry_mutex_.lock();
  ...
  private:
    ...
    static std::pair<bool, const Entry *> GetEntry(const std::string &key, Args... args) {
        registry_mutex_.lock();
...
template <typename T, class... Args> std::mutex Registry<T, Args...>::registry_mutex_;
...
//*****************************************************************************
template <typename Trait, typename base_type, typename derived_type, typename... Args> struct TypeRegisterer {
    static const typename Registry<base_type, Args...>::Registerer instance;
};

template <typename Trait, typename base_type, typename derived_type, typename... Args>
const typename Registry<base_type, Args...>::Registerer
    TypeRegisterer<Trait, base_type, derived_type, Args...>::instance(
        [](Args... args) { return new derived_type(args...); }, Trait::key(), Trait::file(), Trait::line());
...
#define REGISTER_AT(LINE, KEY, TYPE, ARGS...)                                                                          \
    ...
    const void *CONCAT_TOKENS(_xd_unused, LINE)() const {                                                              \
        return &::factory::TypeRegisterer<CONCAT_TOKENS(_xd_Trait, LINE), TYPE, std::decay<decltype(*this)>::type,     \
                                          ##ARGS>::instance;                                                           \
    }                                                                                                                  \
    static const char *_xd_key(const TYPE *, std::function<void(ARGS)>) { return KEY; }                                \
    static_assert(true, "") // enforce ; at EOL
