...
class ComFactory {
  public:
    ...
    // 注册组件创建函数
    void RegisterComponent(const std::string &name, std::function<std::unique_ptr<ComponentInterface>()> creator);
  ...
  private:
    ...
    std::unordered_map<std::string, std::function<std::unique_ptr<ComponentInterface>()>> creators_;
};