...
def apollo_cc_library(**kwargs):
    ...
    includr_prefix_copts = ["-I" + INCLUDE_PREFIX] + kwargs.pop("copts", [])
    cc_binary(**dict(
        bin_kwargs,
...
def pnc_proto_library(tags = [], **kwargs):
    proto_rule_name = kwargs["name"]
    if not proto_rule_name.endswith("proto"):
        fail("The name of the pnc_proto_library instance must end with \"proto\", e.g. \"example_proto\".")
    cc_proto_rule_name = proto_rule_name.replace("proto", "cc_proto")
    py_proto_rule_name = proto_rule_name.replace("proto", "py_proto")
